...
describe('collection2', function () {
    ...
    it('handles prototype-less objects', async function () {
        const prototypelessTest = new Mongo.Collection('prototypelessTest', Meteor.isClient ? { connection: null } : undefined);
        ...
        await prototypelessTest.insertAsync(prototypelessObject)
    });

    if (Meteor.isServer) {
        // https://github.com/aldeed/meteor-collection2/issues/243
        it('upsert runs autoValue only once', async function () {
            const upsertAutoValueTest = new Mongo.Collection('upsertAutoValueTest', Meteor.isClient ? { connection: null } : undefined);
            let times = 0;
            ...
            await upsertAutoValueTest.removeAsync({});

            await upsertAutoValueTest.upsertAsync(
                {
                    foo: 'bar',
                },
                {
        ...
        it('upsert can handle query operators in the selector', async function () {
            ...
            await upsertQueryOperatorsTest.removeAsync({});
            const oneDayInMs = 1000 * 60 * 60 * 24;
            const yesterday = new Date(Date.now() - oneDayInMs);
            const tomorrow = new Date(Date.now() + oneDayInMs);

            const { numberAffected, insertedId } = await upsertQueryOperatorsTest.upsertAsync(
                {
            ...
            const doc = await upsertQueryOperatorsTest.findOneAsync();
        ...
        it('upsert with schema can handle query operator which contains undefined or null', async function () {
            const upsertQueryOperatorUndefinedTest = new Mongo.Collection(
                'upsertQueryOperatorUndefinedTest', Meteor.isClient ? { connection: null } : undefined
            );
            ...
            await upsertQueryOperatorUndefinedTest.removeAsync({});

            const result = await upsertQueryOperatorUndefinedTest.upsertAsync(
                {
                    foo: undefined,
                },
                {
            ...
            const doc = await upsertQueryOperatorUndefinedTest.findOneAsync();
            ...
            const result2 = await upsertQueryOperatorUndefinedTest.upsertAsync(
                {
                    foo: null,
                },
                {
            ...
            const doc2 = await upsertQueryOperatorUndefinedTest.findOneAsync();
        ...
        it('upsert with schema can handle query operator "eq" correctly in the selector when property is left out in $set or $setOnInsert', async function () {
            const upsertQueryOperatorEqTest = new Mongo.Collection(
                'upsertQueryOperatorEqTest', Meteor.isClient ? { connection: null } : undefined
            );
            ...
            await upsertQueryOperatorEqTest.removeAsync({});

            const result = await upsertQueryOperatorEqTest.upsertAsync(
                {
                    foo: { $eq: 'test' },
                },
                {
            ...
            expect(result.numberAffected).toBe(1);
            const doc = await upsertQueryOperatorEqTest.findOneAsync();
            expect(result.insertedId).toBe(doc._id);
        ...
        it('upsert with schema can handle query operator "in" with one element correctly in the selector when property is left out in $set or $setOnInsert', async function () {
            const upsertQueryOperatorInSingleTest = new Mongo.Collection(
                'upsertQueryOperatorInSingleTest', Meteor.isClient ? { connection: null } : undefined
            );
            ...
            await upsertQueryOperatorInSingleTest.removeAsync({});

            const result = await upsertQueryOperatorInSingleTest.upsertAsync(
                {
                    foo: { $in: ['test'] },
                },
                {
            ...
            expect(result.numberAffected).toBe(1);
            const doc = await upsertQueryOperatorInSingleTest.findOneAsync();
            expect(result.insertedId).toBe(doc._id);
            expect(doc.foo).toBe('test');
            expect(doc.bar).toBe(2);
            expect(doc.baz).toBe(4);
        ...
        it('upsert with schema can handle query operator "in" with multiple elements correctly in the selector when property is left out in $set or $setOnInsert', async function () {
            const upsertQueryOperatorInMultiTest = new Mongo.Collection(
                'upsertQueryOperatorInMultiTest', Meteor.isClient ? { connection: null } : undefined
            );
            ...
            await upsertQueryOperatorInMultiTest.removeAsync({});

            const result = await upsertQueryOperatorInMultiTest.upsertAsync(
                {
                    foo: { $in: ['test', 'test2'] },
                },
                {
            ...
            expect(result.numberAffected).toBe(1);
            const doc = await upsertQueryOperatorInMultiTest.findOneAsync();
            expect(result.insertedId).toBe(doc._id);
            expect(doc.foo).toBe(undefined);
            expect(doc.bar).toBe(2);
            expect(doc.baz).toBe(4);
        ...
        it('upsert with schema can handle nested objects correctly', async function () {
            const upsertQueryOperatorNestedObject = new Mongo.Collection(
                'upsertQueryOperatorNestedObject', Meteor.isClient ? { connection: null } : undefined
            );
            ...
            await upsertQueryOperatorNestedObject.removeAsync({});

            const testDateValue = new Date();
            const result = await upsertQueryOperatorNestedObject.upsertAsync(
                {
                    test: '1',
                },
                {
            ...
            const doc = await upsertQueryOperatorNestedObject.findOneAsync({
        ...
        it('upsert with schema can handle query operator "$and" including inner nested selectors correctly when properties is left out in $set or $setOnInsert', async function () {
            const upsertQueryOperatorAndTest = new Mongo.Collection(
                'upsertQueryOperatorAndTest', Meteor.isClient ? { connection: null } : undefined
            );
            ...
            await upsertQueryOperatorAndTest.removeAsync({});

            const result = await upsertQueryOperatorAndTest.upsertAsync(
                {
                    foo: 'test',
            ...
            expect(result.numberAffected).toBe(1);
            const doc = await upsertQueryOperatorAndTest.findOneAsync();
            expect(result.insertedId).toBe(doc._id);
            expect(doc.foo).toBe('test');
    ...
    it('no errors when using a schemaless collection', async function () {
        ...
        const newId = await noSchemaCollection.insertAsync(
            {
                a: 1,
                b: 2,
            });
        ...
        const doc = await noSchemaCollection.findOneAsync(newId);
        expect(doc instanceof Object).toBe(true);
        expect(doc.userFoo).toBe('userBar');

        await noSchemaCollection.updateAsync(
            {
                _id: newId,
            },
            {
    ...
    it('empty strings are removed but we can override', async function () {
        ...
        const newId1 = await RES.insertAsync(
            {
                foo: 'foo',
                bar: '',
            });
        ...
        const doc = await RES.findOneAsync(newId1);
        expect(doc instanceof Object).toBe(true);
        expect(doc.bar).toBe(undefined);

        // Don't remove empty strings
        const newId2 = await RES.insertAsync(
            {
                foo: 'foo',
                bar: '',
            },
        ...
        const doc3 = await RES.findOneAsync(newId1);
    ...
    it('extending a schema after attaching it, collection2 validation respects the extension', async function () {
        ...
        const collection = new Mongo.Collection('ExtendAfterAttach', Meteor.isClient ? { connection: null } : undefined);
    ...
    it('extending a schema with a selector after attaching it, collection2 validation respects the extension', async () => {
        ...
        const collection = new Mongo.Collection('ExtendAfterAttach2', Meteor.isClient ? { connection: null } : undefined);
    ...
    it('pick or omit schema fields when options are provided', async function () {
        ...
        const collection = new Mongo.Collection('pickOrOmit', Meteor.isClient ? { connection: null } : undefined);
        ...
        const newId2 = await collection.insertAsync({ bar: 'test' }, { omit: ['foo'] });

        expect(typeof newId2).toBe('string');

        const doc = await collection.findOneAsync(newId2);
        expect(doc instanceof Object).toBe(true);
        expect(doc.foo).toBe(undefined);
        ...
        const doc2 = await collection.findOneAsync(newId2);
        expect(doc2 instanceof Object).toBe(true);
